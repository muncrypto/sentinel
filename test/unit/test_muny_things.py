...
@pytest.fixture
def valid_mun_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Ui' if (network == 'testnet') else 'MCvdjCNHZPdgvjCDj9QsW2pw5gPPVt6MXt'


@pytest.fixture
def invalid_mun_address(network='mainnet'):
    return 'yYe8KwyaUu5YswSYmB3q3ryx8XTUu9y7Uj' if (network == 'testnet') else 'MCvdjCNHZPdgvjCDj9QsW2pw5gPPVt6MXc'
...
def test_valid_mun_address():
    from munlib import is_valid_mun_address

    main = valid_mun_address()
    test = valid_mun_address('testnet')

    assert is_valid_mun_address(main) is True
    assert is_valid_mun_address(main, 'mainnet') is True
    assert is_valid_mun_address(main, 'testnet') is False

    assert is_valid_mun_address(test) is False
    assert is_valid_mun_address(test, 'mainnet') is False
    assert is_valid_mun_address(test, 'testnet') is True


def test_invalid_mun_address():
    from munlib import is_valid_mun_address

    main = invalid_mun_address()
    test = invalid_mun_address('testnet')

    assert is_valid_mun_address(main) is False
    assert is_valid_mun_address(main, 'mainnet') is False
    assert is_valid_mun_address(main, 'testnet') is False

    assert is_valid_mun_address(test) is False
    assert is_valid_mun_address(test, 'mainnet') is False
    assert is_valid_mun_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from munlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % munlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(munlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(munlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(munlib.blocks_to_seconds(16616)) == 2612035
