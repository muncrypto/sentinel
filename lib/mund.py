...
class MunDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some mund methods
    ...
    @classmethod
    def from_mun_conf(self, mun_dot_conf):
        from mun_config import MunConfig
        config_text = MunConfig.slurp_config_file(mun_dot_conf)
        creds = MunConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = munlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = munlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = munlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_mund_proto_version_with_sentinel_ping)
