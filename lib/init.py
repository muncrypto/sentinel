...
def has_mun_conf():
    ...
    valid_mun_conf = False

    # ensure mun_conf exists & readable
    #
    # if not, print a message stating that Mun Core must be installed and
    # configured, including JSONRPC access in mun.conf
    try:
        f = io.open(config.mun_conf)
        valid_mun_conf = True
    except IOError as e:
        print(e)

    return valid_mun_conf
...
def main():
    ...
    if not has_mun_conf():
        print("MunCore must be installed and configured, including JSONRPC access in mun.conf")
        sys.exit(1)
