...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, mund):
        golist = mund.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_mund(mund, item)
    ...
    @classmethod
    def import_gobject_from_mund(self, mund, rec):
        import decimal
        import munlib
        import inflection
        ...
        # shim/mund conversion
        object_hex = munlib.SHIM_deserialise_from_mund(object_hex)
        objects = munlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from mund...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from mund, with every run
        ...
        # get/create, then sync payment amounts, etc. from mund - Mund is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(mund)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from mund! %s" % e)
            govobj.vote_delete(mund)
            return (govobj, None)
    ...
    def vote_delete(self, mund):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(mund, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, mund, signal, outcome):
        import munlib
        ...
        output = mund.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = munlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(mund, signal)

    def sync_network_vote(self, mund, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = mund.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = MUND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 mun addr, non-multisig
            if not munlib.is_valid_mun_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Mun address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (munlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MunDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import munlib
        obj_data = munlib.SHIM_serialise_for_mund(self.serialise())
    ...
    def prepare(self, mund):
        try:
            object_hash = mund.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = MUND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not munlib.is_valid_mun_address(addr, config.network):
    ...
    def hash(self):
        import munlib
        return munlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = MUND_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, mund):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - mund.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, mund):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - mund.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, mund):
        now = int(time.time())
        return (self.created_at < (now - mund.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, mund):
        if self.is_expired(mund):
            return False

        return True

    def is_deletable(self, mund):
        if self.is_expired(mund):
            return True
