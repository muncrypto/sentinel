...
def is_valid_mun_address(address, network='mainnet'):
    ...
    mun_version = 140 if network == 'testnet' else 50
    ...
    try:
        ...
        # rescue from exception, not a valid Mun address
        return False

    if (address_version != mun_version):
        return False
...
# shims 'til we can fix the mund side
def SHIM_serialise_for_mund(sentinel_hex):
    from models import MUND_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = MUND_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in mund
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # mund expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    mund_hex = serialise(obj)
    return mund_hex


# shims 'til we can fix the mund side
def SHIM_deserialise_from_mund(mund_hex):
    from models import MUND_GOVOBJ_TYPES

    # unpack
    obj = deserialise(mund_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('mun.conf')
